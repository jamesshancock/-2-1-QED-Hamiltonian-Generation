...
def U_term_n(hamiltonian, lattice, n, direction): 
    index = lattice.labels[n]
    link_index = lattice.link_indexing[(index, direction)]
...
def magnetic_term_n(hamiltonian, lattice, n, g, a):
    ...
    coeff = -1/(2*(a**2)*(g**2))
    for i in range(4): Us[i] = U_term_n(Us[i], lattice, ns_directions[i][0], ns_directions[i][1])
...
def kinetic_subterm_n(lattice, n, direction):
    ...
    if indices_mu not in list(lattice.reverse_labels.keys()):
        raise ValueError(f"indiced_mu = {indices_mu} not in our lattice")
    ...
    U_hamiltonian = U_term_n(U_hamiltonian, lattice, n, direction)
...
def kinetic_term_n(hamiltonian, lattice, n, a):
    ...
    for direction in lattice.directions[n]:
        temp_hamiltonian = kinetic_subterm_n(lattice, n, direction)
...
def generate_qed_hamiltonian(parameters):
    ...
    for n in range(lattice.n_links):
        hamiltonian = electric_field_term_n(hamiltonian, lattice, n, parameters['g'])

    # Magnetic field term
    for n in lattice.plaquettes:
        hamiltonian = magnetic_term_n(hamiltonian, lattice, n, parameters['g'], parameters['a'])
    
    # Kinetic term
    for n in range(lattice.n_fermion_qubits):
        hamiltonian = kinetic_term_n(hamiltonian, lattice, n, parameters['a'])
